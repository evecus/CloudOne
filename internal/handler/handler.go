...
// ── FetchURL（SSRF 防护加强版 + GitHub 代理可选加速）──────────────────────────
...
func (h *Handler) FetchURL(c *gin.Context) {
	...
	// 读取“GitHub 加速下载”开关：开启且是 GitHub 链接时，
	...
	if s.GithubProxyEnabled && isGithubURL(req.URL) {
		ranked := rankGithubProxies(ctx, req.URL)
		if len(ranked) == 0 {
			c.JSON(500, gin.H{"error": "no available github proxy, please try again later or disable acceleration"})
			return
		}
...
func (h *Handler) TerminalWebSocket(c *gin.Context) {
	// 前端会带上当前浏览的虚拟目录（?cwd=/etc），据此换算为磁盘真实路径，
	// 让终端打开时就位于用户当前所在目录，而不是固定的存储根目录。
	cwd := ""
	if v := c.Query("cwd"); v != "" {
		if abs, err := h.files.SafeAbsPath(v); err == nil {
			cwd = abs
		}
	}
