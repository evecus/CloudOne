    ...
    <div v-if="modelValue" class="modal-bg" @mousedown="onOverlayMousedown" @mouseup="e=>onOverlayMouseup(e,()=>$emit('update:modelValue',false))">
      <div class="settings-modal">
        ...
        <div class="sm-body">
          ...
          <!-- GitHub 加速下载 -->
          <div class="sm-card">
            <h4 class="sm-card-title">{{ lang === 'zh' ? '远程下载' : 'Remote Download' }}</h4>
            <div class="sm-field">
              <label style="display:flex;align-items:center;justify-content:space-between;cursor:pointer">
                <span style="font-size:13px;font-weight:500;color:var(--gray-700)">
                  {{ lang === 'zh' ? 'GitHub 加速下载（使用代理站点）' : 'GitHub accelerated download (via proxy)' }}
                </span>
                <div class="toggle-wrap" @click="toggleGithubProxy">
                  <div class="toggle" :class="{ on: githubProxyEnabled }">
                    <div class="toggle-knob"></div>
                  </div>
                </div>
              </label>
              <p class="sm-desc">
                {{ lang === 'zh'
                  ? '开启后，远程获取 GitHub 相关链接时会自动探测并使用最快的代理站点下载；关闭则始终直连（默认关闭）。'
                  : 'When enabled, GitHub-related links will be downloaded via the fastest available proxy. When disabled (default), downloads always connect directly.' }}
              </p>
            </div>
          </div>
...
async function toggleGithubProxy() {
  githubProxyEnabled.value = !githubProxyEnabled.value
  try { await api.put('/settings', { github_proxy_enabled: githubProxyEnabled.value }) } catch {}
}
...
async function loadData() {
  ...
  try {
    ...
    // GitHub 加速下载：默认关闭，仅以服务端为准
    githubProxyEnabled.value = !!data.github_proxy_enabled
...
.sm-desc { color: var(--gray-500); font-size: 12px; margin-top: 2px; line-height: 1.5; }
</style>
