  ...
  <div class="editor-page">
    ...
    <div class="editor-header">
      ...
      <div class="editor-header-right">
        ...
        <!-- 预览模式：显示"编辑"按钮，进入可编辑模式 -->
        <button v-if="(mode === 'text' || forceText) && isPreviewing" class="btn-edit-mode" @click="enterEditMode" :title="lang==='zh'?'编辑文件':'Edit file'">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 013 3L7 19l-4 1 1-4 12.5-12.5z"/>
          </svg>
          <span class="btn-edit-label">{{ lang==='zh' ? '编辑' : 'Edit' }}</span>
        </button>
        <!-- 编辑模式：显示"返回预览"与"保存" -->
        <button v-if="(mode === 'text' || forceText) && !isPreviewing" class="btn-ghost-sm btn-back-preview" @click="exitEditMode" :title="lang==='zh'?'返回预览':'Back to preview'">
          {{ lang==='zh' ? '返回预览' : 'Preview' }}
        </button>
        <button v-if="(mode === 'text' || forceText) && !isPreviewing" class="btn-save" @click="doSave" :disabled="saving">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
const saveError = ref('')
// 打开文件默认是预览（只读）模式，需手动点击"编辑"才进入可编辑模式
...
async function loadFile() {
  ...
  contentError.value = ''
  previewUrl.value = ''
  // 新建文件跳转过来时带 ?edit=1，直接进入编辑模式；
  // 普通打开则保持默认的预览模式（需手动点击"编辑"才可改）。
  isPreviewing.value = route.query.edit === '1' ? false : true
...
function onKeydown(e) {
  if ((e.ctrlKey || e.metaKey) && e.key === 's') {
    e.preventDefault()
    if ((mode.value === 'text' || forceText.value) && !isPreviewing.value) doSave()
  }
}
...
.btn-edit-mode:hover { background: var(--gray-50); border-color: var(--blue-300, #93C5FD); color: var(--blue-600, #2563EB); }
.btn-edit-mode svg { width: 15px; height: 15px; flex-shrink: 0; }
.btn-back-preview { flex-shrink: 0; }
.btn-save {
  display: flex;
  align-items: center;
  gap: 6px;
