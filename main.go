...
func init() {
	...
	// "系统工具能解析、程序内 net.LookupHost 却解析失败" 的不一致情况。
	//
	// 注意：此设置只在编译时 CGO_ENABLED=1 才会生效；
	// 若编译时 CGO_ENABLED=0（例如本项目发布的纯 Go 静态版本），
	// Go 运行时没有 cgo resolver 可用，会自动回退为纯 Go resolver，
	// 这行设置不会报错，但也不会有实际效果——因此本项目同时提供
	// CGO_ENABLED=0 和 CGO_ENABLED=1 两个版本的 Release 二进制，
	// 在宿主机直接部署且使用透明代理/fake-ip 时，建议下载 cgo 版本。
